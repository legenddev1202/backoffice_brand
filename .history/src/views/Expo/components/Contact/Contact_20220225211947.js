...
const GridItemFormBlock = () => {
  ...
  const onSubmit = (values) => {
    ...
    //   subject: `Received a message from ${email.value}`,
    //   emailBody: `
    //     <p>Name: ${first.value} ${last.value}</p>
    //     <p>Phone Number: ${phone.value}</p>
    //     <p>Email Address: ${email.value}</p>
    //     <p>Company Name: ${company.value}</p>
    //     <p>Comment: ${product.value}</p>
    //   `
    // }
  
    // const response = await axios.post('https://us-central1-insurancewebapptest.cloudfunctions.net/sendMailOverHTTP', form);
